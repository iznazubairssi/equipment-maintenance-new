...
async function checkDatabase() {
    ...
    try {
        ...
        // 2. Check for CAP-specific tables
        console.log('\n🔍 CHECKING FOR CAP ENTITIES:');
        console.log('============================');
        const capTables = tablesResult.rows.filter(table => 
            table.table_name.includes('equipment') || 
            table.table_name.includes('d4iot')
        );
        
        if (capTables.length > 0) {
            console.log('Found CAP-related tables:');
            capTables.forEach(table => {
                console.log(`  ✅ ${table.table_name}`);
            });
        } else {
            console.log('  ❌ No CAP-related tables found');
        }
        ...
        for (let table of tablesResult.rows) {
            ...
            if (count > 0 && count <= 10) {
                ...
                console.log(`  (showing first 5 of ${count} records)`);
                const dataResult = await client.query(`SELECT * FROM "${table.table_name}" LIMIT 5`);
                dataResult.rows.forEach(row => {
                    console.log('   ', JSON.stringify(row));
                });
